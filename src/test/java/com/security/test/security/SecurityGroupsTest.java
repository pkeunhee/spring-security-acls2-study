...
public class SecurityGroupsTest extends AbstractSecurityTest {
	...
	public void setup() {
		...
		jdbcUserDetailsManager.createGroup(TEST_GROUP, authorities); // 그룹 생성
		userGroupManager.createUserWithAuthoriy(USER_USER, "ROLE_USER"); // 유저 생성
	...
	public void testAddAuthorityToGroup() {
		...
		jdbcUserDetailsManager.addGroupAuthority(TEST_GROUP, authority); // 그룹에 권한을 넣는다. ROLE_TESTE
		List<GrantedAuthority> foundAuthorities = jdbcUserDetailsManager.findGroupAuthorities(TEST_GROUP); // 그룹 권한 목록 가져오기
